...
void noah_windtunnel_controller(float start_time, struct overactuated_mixing_t *output) {
    float current_time = get_sys_time_float();

    if (state * length_test_case > experiment_block_duration) {
        end_experiment = true;
        // Add some kind of experiment ending message
        return
    }

    if (time - start_time < 15) {
        noah_windtunnel_startup(start_time, current_time, output);
    }

    else {
        state = noah_windtunnel_matrix(start_time, current_time, output);
    }
...
void overactuated_mixing_run(void)
{
    ...
    if(autopilot.mode == AP_MODE_NAV || autopilot.mode == AP_MODE_HOVER_DIRECT)
    {
        ...
        if(autopilot.motors_on) {
            ...
            #ifdef TEST_NOAH_WINDTUNNEL
            noah_windtunnel_controller(start_time, &overactuated_mixing)
            #endif
